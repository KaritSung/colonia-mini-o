...
const OtpPage = ({ backwardPage }) => {
  ...
  const onLogin = (values) => {
    const token = "your_generated_token_from_api"; // ใช้ข้อมูลที่ได้รับจาก API

    // บันทึก token ลงใน localStorage
    localStorage.setItem("token", token);

    // เปลี่ยนเส้นทางไปยังหน้า Home หรือหน้าหลักหลังจากเข้าสู่ระบบสำเร็จ
    window.location.href = "/"; // เปลี่ยนเส้นทางไปยังหน้าหลัก
    // หรือ
    // history.push("/"); // หากใช้ react-router-dom
  };
